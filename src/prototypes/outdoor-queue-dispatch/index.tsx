...
const OutdoorQueueDispatch: React.FC = () => {
  ...
  const [logs, setLogs] = useState<ConsoleLog[]>(() => INITIAL_LOGS.map((r) => ({ ...r })));
  ...
  const led = ledText(plantOn, head);
  const latestHit = channelLogs.find((r) => r.plate);

  const pushLog = (row: Omit<ConsoleLog, 'id'>) => {
    setLogs((list) => [{ ...row, id: `l-${Date.now()}-${Math.random()}` }, ...list].slice(0, 120));
  };

  useEffect(() => {
    const timer = window.setInterval(() => {
      const plate = PASSING_PLATES[passIdxRef.current % PASSING_PLATES.length];
      passIdxRef.current += 1;
      const time = formatNow();
      const channelDevices = DEVICES.filter((d) => d.channelId === channelId);
      if (!devicesOnline(channelDevices)) {
        pushLog({
          channelId,
          time,
          message: '设备异常离线无法正常排队请检查',
          level: 'error',
        });
        return;
      }
      if (!plantOn) {
        pushLog({
          ...
          time,
          plate,
          message: `${plate} 已识别，排队程序未启用`,
          level: 'info',
        });
        return;
      }
      const list = queueRef.current;
      if (list.some((v) => v.channelId === channelId && v.plate === plate)) {
        pushLog({
          channelId,
          time,
          plate,
      ...
      setQueue([...others, ...renumber([...active, added, ...frozen])]);
      pushLog({
        channelId,
        time,
        plate,
  ...
  const handlePlant = async (on: boolean) => {
    setPlantOn(on);
    const time = formatNow();
    if (on) {
      const res = await closeYunyiQueueApi();
      message.success('厂外自动排队已启用');
      pushLog({ channelId: channel.id, time, message: res.message, level: 'info' });
      return;
    }
    message.success('厂外自动排队已停用');
    pushLog({ channelId: channel.id, time, message: '厂外自动排队已停用', level: 'info' });
  };
  ...
  const handleAdd = (plate: string) => {
    ...
    pushLog({ channelId: channel.id, time, plate, message: `${plate} 排队成功`, level: 'info' });
  ...
  const handleFreeze = (id: string) => {
    ...
    patchChannel(regroup([...rest, { ...target, status: 'frozen', allowEntry: false }]));
      ...
      <div className="oqd-root">
        <header className="oqd-head">
          ...
          <div className="oqd-head-tools">
            <Tag color={plantOn ? 'success' : 'warning'}>{plantOn ? '自动取号' : '排队停用'}</Tag>
        ...
        <div className="oqd-body">
          <div className="oqd-left">
            <VideoMonitor
              channel={channel}
              plate={latestHit?.plate}
              recognizedAt={latestHit?.time}
              led={led}
              live
            />
