 ...
 * 口径：班值报表 / 上煤单 / coal_heap / 卸煤·上煤单耗 / 发电成本 / 设备投运率
 */
const KPIS: KpiItem[] = [
  {
    key: 'in',
...
/** 来煤接卸动态：船次统计 + 卸煤管理中「正在卸煤」状态的卸煤信息 */
const SHIP_STATS = { onWay: 2, atBerth: 1 };

const UNLOADING = {
  ship: '新一海17',
  voyage: '2613',
  planNo: '2613-PD2026061801',
  coal: '国信1-4500（4500-5000kcal/kg）',
  planTon: 49936,
  doneTon: 29569,
  type: '皮带卸煤（煤场）· A入厂皮带秤',
};
...
interface DeviceStatusCounts {
  /** 入厂采样装置运行：In_Belt_Sample_On=1 */
  sampleRun: number;
  /** 入厂采样装置停止：In_Belt_Sample_On=0（需求原文停止亦写为=1，按停止语义取 0） */
  sampleStop: number;
...
const COMPETITION_TOP3 = [
  { rank: '第一名', name: '丙值', score: 86.42, cls: 'r1', icon: <CrownFilled className="icon" /> },
  { rank: '第二名', name: '甲值', score: 79.18, cls: 'r2', icon: <TrophyFilled className="icon" /> },
  { rank: '第三名', name: '戊值', score: 75.66, cls: 'r3', icon: <TrophyFilled className="icon" /> },
];
...
const buildStockPieOption = (): echarts.EChartsOption => ({
  tooltip: { trigger: 'item', formatter: '{b}：{c} 万t（{d}%）' },
  legend: { bottom: 0, textStyle: { fontSize: 12 }, itemWidth: 12, itemHeight: 8 },
  series: [
    {
      type: 'pie',
      radius: ['42%', '66%'],
      ...
      data: [
        { name: '神华混煤', value: 12.4, itemStyle: { color: '#2563eb' } },
        { name: '印尼褐煤', value: 7.2, itemStyle: { color: '#0d9488' } },
        { name: '晋北贫瘦煤', value: 5.9, itemStyle: { color: '#e8a13a' } },
        { name: '澳洲烟煤', value: 3.9, itemStyle: { color: '#7c3aed' } },
        { name: '其他煤种', value: 3.2, itemStyle: { color: '#cbd5e1' } },
      ],
    },
  ],
});
...
const Component: React.FC = () => {
  ...
  const deviceRows = useMemo(() => buildDeviceRows(DEVICE_COUNTS_INIT), []);
  const kpis = KPIS;
  ...
  const pieOption = useMemo(buildStockPieOption, []);

  return (
    <ConfigProvider
      locale={zhCN}
      theme={{ token: { colorPrimary: '#1d4ed8', borderRadius: 8, fontFamily: '"PingFang SC", "Microsoft YaHei", "Segoe UI", sans-serif' } }}
    >
      <div className="pwb-root">
        {/* 欢迎条（无卡片框，轻量） */}
        <div className="pwb-welcome pwb-reveal" style={{ ['--delay' as string]: '40ms' }}>
        ...
        {/* 核心指标（左，整块面板） + 功能模块（右） */}
        <div className="pwb-top-grid pwb-reveal" style={{ ['--delay' as string]: '100ms' }}>
        ...
        {/* 主体：安全预警中心 + 待办 */}
        <div className="pwb-main-grid pwb-reveal" style={{ ['--delay' as string]: '170ms' }}>
        ...
        {/* 生产作业动态 */}
        <div className="pwb-prod-grid pwb-reveal" style={{ ['--delay' as string]: '240ms' }}>
          <div className="pwb-card">
            <div className="pwb-section-hd">
              <div className="pwb-section-title">
                当前配煤单
              ...
              <Button
                type="link"
                size="small"
                onClick={() => gotoModule('配煤单管理', '/prototypes/coal-blending-bill-management/')}
              >
                配煤单管理 <RightOutlined />
              </Button>
            </div>
            <div className="pwb-workorder-body">
              <div className="pwb-bill-hero">
                <div className="pwb-bill-name">260724加仓计划</div>
                <div className="pwb-bill-no">GXSZ-26072401</div>
              </div>
              <div className="pwb-mini-stats">
                <div className="pwb-mini-stat">
                  <b>37</b>
                  <span>配煤明细</span>
                </div>
                <div className="pwb-mini-stat">
              ...
              <div className="pwb-bill-foot">创建人 · 发电部燃料专工</div>
            </div>
          </div>

          <div className="pwb-card">
            <div className="pwb-section-hd">
              <div className="pwb-section-title">来煤接卸动态</div>
              <span className="pwb-hd-stats">
                在途船次 <b>{SHIP_STATS.onWay}</b>
                <i />
                在泊船次 <b className="at-berth">{SHIP_STATS.atBerth}</b>
              </span>
            </div>
            <div className="pwb-unloading-body">
              <div className="pwb-bill-hero">
                <div className="pwb-bill-name">
                  {UNLOADING.ship} · 航次 {UNLOADING.voyage}
                  <Tag color="green" style={{ marginInlineStart: 8, marginInlineEnd: 0 }}>
                    正在卸煤
                  </Tag>
                </div>
                <div className="pwb-bill-no">{UNLOADING.planNo}</div>
              </div>
              <div className="pwb-unload-meta" title={`${UNLOADING.coal} · ${UNLOADING.type}`}>
                {UNLOADING.coal} · {UNLOADING.type}
              </div>
              <div className="pwb-unload-progress">
                <b>
                  {Math.round((UNLOADING.doneTon / UNLOADING.planTon) * 100)}
                  <small>%</small>
                </b>
                <span>
                  已卸 <em>{UNLOADING.doneTon.toLocaleString()}</em> / 计划 {UNLOADING.planTon.toLocaleString()} t
                </span>
              </div>
              <Progress
                percent={Math.round((UNLOADING.doneTon / UNLOADING.planTon) * 100)}
                status="active"
                strokeColor="#1d4ed8"
                showInfo={false}
              />
        ...
        {/* 数据趋势与分析摘要 */}
        <div className="pwb-trend-grid pwb-reveal" style={{ ['--delay' as string]: '310ms' }}>
          <div className="pwb-card">
            <div className="pwb-section-hd">
          ...
          <div className="pwb-card">
            <div className="pwb-section-hd">
              <div className="pwb-section-title">煤场存煤结构</div>
              <span className="pwb-section-extra">合计 32.6 万t</span>
            </div>
            <div style={{ padding: '8px 8px 4px' }}>
